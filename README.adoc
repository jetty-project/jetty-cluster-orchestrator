...
* `LocalLauncher` for local JVM processes 
...
Formatting is enforced by https://github.com/diffplug/spotless[spotless]. A normal build checks it and
fails if something is off, so reformat before you push:

[source,bash]
----
mvn -Pmay-spotless-apply validate
----

Spotless owns the Java formatting (Apache Maven style via `palantirJavaFormat`), the import order,
the license headers, and the ordering of the `pom.xml` files. If you need it to leave a block alone,
fence it with `// spotless:off` and `// spotless:on`.

The formatting was introduced in one mechanical commit, listed in `.git-blame-ignore-revs`. GitHub skips
it in the blame view automatically; to get the same locally, run once per clone:
...
|`jco.zookeeper.retry.baseSleepTimeMs`
|`1000`
|Base delay between ZooKeeper retry attempts in milliseconds
|`-Djco.zookeeper.retry.baseSleepTimeMs=500`

|`jco.zookeeper.retry.maxRetries`
|`3`
|Maximum number of retry attempts for ZooKeeper operations
|`-Djco.zookeeper.retry.maxRetries=5`

|`jco.zookeeper.sessionTimeoutMs`
|`30000`
|ZooKeeper session timeout in milliseconds
|`-Djco.zookeeper.sessionTimeoutMs=15000`

|`jco.zookeeper.connectionTimeoutMs`
|`15000`
|ZooKeeper initial connection timeout in milliseconds
|`-Djco.zookeeper.connectionTimeoutMs=10000`
|===
...
[source,bash]
----
mvn test -Djco.zookeeper.retry.maxRetries=5 -Djco.zookeeper.retry.baseSleepTimeMs=500
----
...
[source,bash]
----
mvn test -Djco.zookeeper.sessionTimeoutMs=10000 -Djco.zookeeper.connectionTimeoutMs=5000
----
