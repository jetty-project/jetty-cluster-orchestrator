...
. Have two `Node` s be part of the node array. The first one is identified by the string `&quot;1&quot;` and is going to run on `localhost`,
...
If you want to use machines over the network, you can simply specify their names instead of `localhost` in the node array:
