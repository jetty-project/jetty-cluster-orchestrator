...
https://kind.sigs.k8s.io[kind] (Kubernetes IN Docker) is the easiest way to get a local cluster for testing.
...
kind create cluster --name jco-test

# Verify kubectl can reach it
kubectl cluster-info --context kind-jco-test
----

Once the cluster is up, run the full build including Kubernetes integration tests:

[source,bash]
----
mvn clean install -Dkubernetes.config.path=$HOME/.kube/config
----

To also specify the container image and namespace:
...
[source,bash]
----
kind delete cluster --name jco-test
----
